...
UniValue prioritisetransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            ...
            "                  (priority of a transaction is calculated: coinage * value_in_uwage / txsize) \n"
            "3. fee delta      (numeric, required) The fee value (in uwage) to add (or subtract, if negative).\n"
